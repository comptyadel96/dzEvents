...
router.get('/', async (req, res) => {
  ...
  const first = await FirstTime.find().skip(startIndex).limit(limit).sort({ createdAt: -1 }).populate('owner', '-password -profilePicture')
...
const upload = multer({
  ...
  fileFilter(req, file, cb) {
    if (!file.originalname.match(/\.(jpg|jpeg|png)$/gi)) {
      return cb(new Error('le type de fichier doit étre une image de taille inférieure ou égale a 1.5MBs'))
    }
    cb(undefined, true)
  },
}).single('firstTimePic')
...
router.post('/', auth,upload, async (req, res) => {
  ...
  if (!user.firstTimePublished) {
    ...
    const buffer = await sharp(req.file.buffer).resize({ width: 350, height: 300 }).png().toBuffer()
    ...
    if (!first) return res.status(400).send('vous devez remplir tous les champs obligatoire')
    res.status(200).send(first)
  } else {
    res.status(403).send('vous avez le droit a une seule publication dans la section premiére fois')
  }
})
...
router.put('/:id', auth, async (req, res) => {
  ...
  if (!req.body.owner || !req.body._id) {
    const first = await FirstTime.findOneAndUpdate({ _id, owner: req.user._id }, { ...req.body }, { runValidators: true, new: true })
    if (!first) return res.status(404).send(' oops ! cet élèment est introuvable :( ')
    await first.save()
    res.status(200).send(first)
  } else {
...
router.delete('/:id', auth, async (req, res) => {
  ...
  if (!first) return res.status(404).send('cet évènement est introuvable ou bien a déja été  supprimer')
...
router.delete('/:id/picture', async (req, res) => {
  const first = await FirstTime.findOne({ _id: req.params.id, owner: req.user._id })
  if (!first || !first.photo) {
